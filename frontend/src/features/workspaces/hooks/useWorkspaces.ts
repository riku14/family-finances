...
import { acceptInvitation, createInvitation, createWorkspace, fetchWorkspaces } from "../api";
...
export const useWorkspaces = () => {
    ...
    const handleAcceptInvitation = async (body: { inviteToken: string }) => {
        await apiUtils(() => acceptInvitation(body), "参加しました", refresh)
    }
    const handleCreateInvitation = async (workspaceId: number) => {
        await apiUtils(() => createInvitation(workspaceId), "招待リンクを発行しました")
    }
    return { loading, error, data, handleCreate, handleAcceptInvitation, handleCreateInvitation }
}