...
// カテゴリ登録
export const createCategory = async (body: { name: string, type: "INCOME" | "EXPENSE", color?: string }) => {
...
// カテゴリ更新
export const updateCategory = async (categoryId: number, body: { name: string, type: "INCOME" | "EXPENSE", color?: string }) => {
    const { data, error } = await client.PUT("/api/workspaces/{workspaceId}/categories/{categoryId}", {
